...
TEST(ServiceTagsTest, CheckStatusDetailed_Tags) {
  ...
  std::string bid_1 = server.registerBranch(request, "post_storage", regions_1, "write_post", "");
  ASSERT_EQ(getFullBid(request->getRid(), 1), bid_1);
  
  utils::Status r = server.checkStatus(request, "post_storage", "", "", true);
  ...
  int found = server.closeBranch(request, parseFullBid(&server, request, bid_1, 1), "EU");
  ASSERT_EQ(1, found);

  r = server.checkStatus(request, "post_storage", "EU", "", true);
  ASSERT_EQ(CLOSED, r.status);
  ASSERT_EQ(1, r.tagged.size());
  ASSERT_EQ(1, r.tagged.count("write_post"));
  ASSERT_EQ(CLOSED, r.tagged["write_post"]);

  r = server.checkStatus(request, "post_storage", "US", "", true);
  ASSERT_EQ(OPENED, r.status);
  ASSERT_EQ(1, r.tagged.size());
  ASSERT_EQ(1, r.tagged.count("write_post"));
  ASSERT_EQ(OPENED, r.tagged["write_post"]);
