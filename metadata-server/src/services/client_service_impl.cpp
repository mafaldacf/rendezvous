...
ClientServiceImpl::ClientServiceImpl(
  ...
  : _server(server), _num_wait_calls(0), _replica_client(addrs), _num_replicas(addrs.size()) {
}

metadata::Request * ClientServiceImpl::getRequest(const std::string& rid, const rendezvous::RequestContext& ctx) {
  ...
  // replicated servers
    ...
    request->getVersionsRegistry()->waitRemoteVersions(ctx);
  }
  return request;
}
...
grpc::Status ClientServiceImpl::RegisterBranchesDatastores(grpc::ServerContext* context, 
  ...
  if (request->branches().size() > 0){
    for (const auto& branches : request->branches()) {
      std::string bid = _server->registerBranches(rdv_request, branches.datastore(), branches.regions(), branches.tag());
      if (bid.empty()) {
  ...
  // client uses same set of regions for all datastores and each datastore has a specific tag
    ...
    for (const auto& datastore: request->datastores()) {
      std::string bid = _server->registerBranches(rdv_request, datastore, regions, tags[i]);
      if (bid.empty()) {
        return grpc::Status(grpc::StatusCode::ALREADY_EXISTS, utils::ERR_MSG_BRANCH_ALREADY_EXISTS);
      }
      response->add_bids(bid);
...
grpc::Status ClientServiceImpl::CloseBranch(grpc::ServerContext* context, 
  ...
  metadata::Request * rdv_request = getRequest(rid, request->context());
...
grpc::Status ClientServiceImpl::WaitRequest(grpc::ServerContext* context, 
  ...
  int result = _server->waitRequest(rdv_request, service, region, async, timeout);
  if (result == 1) {
